...
class TileExtractor:
  def __init__(self, lang: str, tolerance: float = 3):
    """
    Initialize a TitleExtractor object.

    Args:
        lang (str): The language for OCR.
        tolerance (float): The tolerance level for title extraction. Default is 3.
  ...
  def __str__(self) -> str:
    ...
    """
    return f"TitleExtractor(device={self.device}, lang={self.lang}, tolerance={self.tolerance})"
  ...
  def _merge_line_titles(self, ocr_results: List[Dict]) -> str:
    ...
    """
    title = pix2text.merge_line_texts(ocr_results)
    return title
